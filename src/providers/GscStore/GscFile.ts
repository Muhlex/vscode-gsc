...
export class GscFile {
	...
	async getCallableDefsRecursive() {
		...
		const addIncludedDefsRecursive = async (file: GscFile) => {
			for (const includePath of await file.getIncludes()) {
				...
				await addIncludedDefsRecursive(includedFile);
			}
		};

		await addDefs(this);
		await addIncludedDefsRecursive(this);
		return defs;
	}
	...
	async getCallableInstancesDefined() {
		...
		const defsScript = await this.getCallableDefsRecursive();
